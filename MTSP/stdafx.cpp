// stdafx.cpp : 標準インクルード MTSP.pch のみを
...
void ApplicationThreadException(Object^ sender, Threading::ThreadExceptionEventArgs^ e)
{
	...
	MessageBox::Show("突然ですがMTSP終了のお知らせです。", "緊急事態発生");
...
void ApplicationUnhandledException(Object^ sender, UnhandledExceptionEventArgs^ e)
{
	...
	MessageBox::Show("突然ですがMTSP終了のお知らせです。", "例外が飛んできました");
...
void LoadMTOption()
{
	...
	_stprintf_s(ini, _T("%sMTSP.ini"), MTOPTION.PATH);

	MTOPTION.CONNECTION_TYPE = GetPrivateProfileInt(_T("MTSP"), _T("ConnectType"), 0, ini);
	GetPrivateProfileString(_T("MTSP"), _T("ConnectIP"),    _T(""),           MTOPTION.CONNECTION_IP,  MAX_NAME,  ini);
	GetPrivateProfileString(_T("MTSP"), _T("Welcome"),      _T(""),           MTOPTION.WELCOME,        MAX_TITLE, ini);
	GetPrivateProfileString(_T("MTSP"), _T("GameExe"),      _T("game.exe"),   MTOPTION.GAME_EXE,      _MAX_PATH,  ini);
	GetPrivateProfileString(_T("MTSP"), _T("ReplayFolder"), _T("Replay"),     MTOPTION.REPLAY_FOLDER, _MAX_PATH,  ini);
	GetPrivateProfileString(_T("MTSP"), _T("VSSound"),      _T("vs.wav"),     MTOPTION.VS_SOUND,      _MAX_PATH,  ini);
	GetPrivateProfileString(_T("MTSP"), _T("NoticeSound"),  _T("notice.wav"), MTOPTION.NOTICE_SOUND,  _MAX_PATH,  ini);
	GetPrivateProfileString(_T("MTSP"), _T("EnterSound"),   _T("enter.wav"),  MTOPTION.ENTER_SOUND,   _MAX_PATH,  ini);
	GetPrivateProfileString(_T("MTSP"), _T("Name"),         _T("名無しさん"), MTOPTION.NAME,           MAX_NAME,  ini);
	GetPrivateProfileString(_T("MTSP"), _T("Comment"),      _T(""),           MTOPTION.COMMENT,        MAX_NAME,  ini);
	MTOPTION.PORT               = GetPrivateProfileInt(_T("MTSP"), _T("Port"),          7500, ini);
	MTOPTION.OPEN_PORT          = GetPrivateProfileInt(_T("MTSP"), _T("OpenPort"),      7500, ini);
	MTOPTION.DNS                = GetPrivateProfileInt(_T("MTSP"), _T("DNS"),              0, ini) == 1 ? true : false;
	MTOPTION.AUTO_SAVE          = GetPrivateProfileInt(_T("MTSP"), _T("AutoSave"),         0, ini);
	MTOPTION.MAX_CONNECTION     = GetPrivateProfileInt(_T("MTSP"), _T("MaxConnection"),    1, ini);
	MTOPTION.BGM_VOLUME         = GetPrivateProfileInt(_T("MTSP"), _T("BGMVolume"),      100, ini);
	MTOPTION.SE_VOLUME          = GetPrivateProfileInt(_T("MTSP"), _T("SEVolume"),       100, ini);
	MTOPTION.MAX_STAGE          = GetPrivateProfileInt(_T("MTSP"), _T("MaxStage"),         1, ini);
	MTOPTION.STAGE_SELECT       = GetPrivateProfileInt(_T("MTSP"), _T("StageSelect"),      0, ini);
	MTOPTION.ROUND              = GetPrivateProfileInt(_T("MTSP"), _T("Round"),            2, ini);
	MTOPTION.TIMER              = GetPrivateProfileInt(_T("MTSP"), _T("Timer"),            0, ini);
	MTOPTION.SIMULATE_DELAY     = GetPrivateProfileInt(_T("MTSP"), _T("SimulateDelay"),    0, ini);
	MTOPTION.HIT_JUDGE          = GetPrivateProfileInt(_T("MTSP"), _T("HitJudge"),         0, ini) == 1 ? true : false;
	MTOPTION.DISPLAY_NAME       = GetPrivateProfileInt(_T("MTSP"), _T("DisplayName"),      0, ini) == 1 ? true : false;
	MTOPTION.DISPLAY_VERSUS     = GetPrivateProfileInt(_T("MTSP"), _T("DisplayVersus"),    0, ini) == 1 ? true : false;
	MTOPTION.DISPLAY_FRAMERATE  = GetPrivateProfileInt(_T("MTSP"), _T("DisplayFramerate"), 0, ini) == 1 ? true : false;
	MTOPTION.DISPLAY_RAND       = GetPrivateProfileInt(_T("MTSP"), _T("DisplayRand"),      0, ini) == 1 ? true : false;
	MTOPTION.REPLAY_DIVIDE      = GetPrivateProfileInt(_T("MTSP"), _T("ReplayDivide"),     0, ini) == 1 ? true : false;
	MTOPTION.CHANGE_WINDOW_SIZE = GetPrivateProfileInt(_T("MTSP"), _T("ChangeWindowSize"), 0, ini) == 1 ? true : false;
	MTOPTION.CHAT_HISTORY       = GetPrivateProfileInt(_T("MTSP"), _T("ChatHistory"),     20, ini);
	MTOPTION.DELAY              = GetPrivateProfileInt(_T("MTSP"), _T("Delay"),            0, ini);
	MTOPTION.INTERVAL           = GetPrivateProfileInt(_T("MTSP"), _T("Interval"),         1, ini);
	MTOPTION.REPLAY_VERSION     = GetPrivateProfileInt(_T("MTSP"), _T("ReplayVersion"),    2, ini);
	MTOPTION.RECORD_REPLAY      = GetPrivateProfileInt(_T("MTSP"), _T("RecordReplay"),     0, ini) == 1 ? true : false;
	MTOPTION.ALLOW_SPECTATOR    = GetPrivateProfileInt(_T("MTSP"), _T("AllowSpectator"),   0, ini) == 1 ? true : false;
	MTOPTION.AFTER_REST         = GetPrivateProfileInt(_T("MTSP"), _T("AfterRest"),        0, ini) == 1 ? true : false;
...
void SaveMTOption()
{
	...
	WritePrivateProfileString(_T("MTSP"), _T("ConnectType"),  buf,                    ini);
	WritePrivateProfileString(_T("MTSP"), _T("ConnectIP"),    MTOPTION.CONNECTION_IP, ini);
	WritePrivateProfileString(_T("MTSP"), _T("Welcome"),      MTOPTION.WELCOME,       ini);
	WritePrivateProfileString(_T("MTSP"), _T("GameExe"),      MTOPTION.GAME_EXE,      ini);
	WritePrivateProfileString(_T("MTSP"), _T("ReplayFolder"), MTOPTION.REPLAY_FOLDER, ini);
	WritePrivateProfileString(_T("MTSP"), _T("VSSound"),      MTOPTION.VS_SOUND,      ini);
	WritePrivateProfileString(_T("MTSP"), _T("NoticeSound"),  MTOPTION.NOTICE_SOUND,  ini);
	WritePrivateProfileString(_T("MTSP"), _T("EnterSound"),   MTOPTION.ENTER_SOUND,   ini);
	WritePrivateProfileString(_T("MTSP"), _T("Name"),         MTOPTION.NAME,          ini);
	WritePrivateProfileString(_T("MTSP"), _T("Comment"),      MTOPTION.COMMENT,       ini);
	_itot_s(MTOPTION.PORT, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("Port"), buf, ini);
	_itot_s(MTOPTION.OPEN_PORT, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("OpenPort"), buf, ini);
	_itot_s(MTOPTION.DNS, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("DNS"), buf, ini);
	_itot_s(MTOPTION.AUTO_SAVE, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("AutoSave"), buf, ini);
	_itot_s(MTOPTION.MAX_CONNECTION, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("MaxConnection"), buf, ini);
	_itot_s(MTOPTION.BGM_VOLUME, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("BGMVolume"), buf, ini);
	_itot_s(MTOPTION.SE_VOLUME, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("SEVolume"), buf, ini);
	_itot_s(MTOPTION.MAX_STAGE, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("MaxStage"), buf, ini);
	_itot_s(MTOPTION.STAGE_SELECT, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("StageSelect"), buf, ini);
	_itot_s(MTOPTION.ROUND, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("Round"), buf, ini);
	_itot_s(MTOPTION.TIMER, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("Timer"), buf, ini);
	_itot_s(MTOPTION.SIMULATE_DELAY, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("SimulateDelay"), buf, ini);
	_itot_s(MTOPTION.HIT_JUDGE, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("HitJudge"), buf, ini);
	_itot_s(MTOPTION.DISPLAY_NAME, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("DisplayName"), buf, ini);
	_itot_s(MTOPTION.DISPLAY_VERSUS, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("DisplayVersus"), buf, ini);
	_itot_s(MTOPTION.DISPLAY_FRAMERATE, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("DisplayFramerate"), buf, ini);
	_itot_s(MTOPTION.DISPLAY_RAND, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("DisplayRand"), buf, ini);
	_itot_s(MTOPTION.REPLAY_DIVIDE, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("ReplayDivide"), buf, ini);
	_itot_s(MTOPTION.CHANGE_WINDOW_SIZE, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("ChangeWindowSize"), buf, ini);
	_itot_s(MTOPTION.CHAT_HISTORY, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("ChatHistory"), buf, ini);
	_itot_s(MTOPTION.DELAY, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("Delay"), buf, ini);
	_itot_s(MTOPTION.INTERVAL, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("Interval"), buf, ini);
	_itot_s(MTOPTION.REPLAY_VERSION, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("ReplayVersion"), buf, ini);
	_itot_s(MTOPTION.RECORD_REPLAY, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("RecordReplay"), buf, ini);
	_itot_s(MTOPTION.ALLOW_SPECTATOR, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("AllowSpectator"), buf, ini);
	_itot_s(MTOPTION.AFTER_REST, buf, 10);
	WritePrivateProfileString(_T("MTSP"), _T("AfterRest"), buf, ini);
...
{
	// 内緒
	return Int64(Net::IPAddress::Parse(ip)->Address).ToString();
}

// IPの複合化
_int64 DecryptionIP(String^ cipher_ip)
{
	return Convert::ToInt64(cipher_ip);
}